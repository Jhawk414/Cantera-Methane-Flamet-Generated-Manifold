...
"Author: Ryan Medlin"
print("This code develops a Methane-Air flamelet-generated manifold (FGM) using the GRI-Mech 1.2 Chemical Reaction Mechanism. Please ensure you have read the included user-guide.\n")
...
manifold_range = [0.4, 0.5, 1, 3, 5, 7, 9, 10] # Range of Physical parameters to simulate each flamelet to build manifold.

# String with all 32 species (GRI 1.2 only)
spec_list = ["H2", "H", "O", "O2", "OH", "H2O", "HO2", "H2O2", "C", "CH", "CH2", "CH2(S)", "CH3", "CH4", "CO", "CO2", "HCO", "CH2O", "CH2OH", "CH3O", "CH3OH", "C2H", "C2H2", "C2H3", "C2H4", "C2H5", "C2H6", "HCCO", "CH2CO", "HCCOH", "N2", "AR"]
...
for i in manifold_range:
    ...
    #==== Domain Details
    domain_width = 0.02
    gas = ct.Solution('grimech12.yaml') # Instantiates a "gas" object
    ...
    PV = flamelet.Y[gas.species_index('CO2')] + flamelet.Y[gas.species_index('H2O')] # Y_CO2 + Y_H2O
    Temp = flamelet.T
    density = flamelet.density
    
    #===== Interpolate Output on evenly-spaced f =====
    new_Z = np.linspace(0.01, 0.999, 75) # Define evenly-spaced mixture fraction (Z)
    ...
    # >> Repeat above by looping through the 32 species' mass fractions.
    Yholder = {}
    z_Yapprox_holder = {}
    interp_Y_hold = {}
    ...
    for k in spec_list:
        Yholder['Y_' + str(k)] = flamelet.Y[gas.species_index(str(k))] # extracts all 32 raw mass fraction solutions into dictionary
        ...
        Y_C_holder['Y_'+str(k)+'_C_group'] = np.column_stack((interp_Y_hold['interp_Y_'+str(k)] , interp_PV)) # groups arrays of Y and C for the i^th flamelet


   # Generate solution table of [Z | φ C | φ C | φ C | φ C], where φ and C are the mixture property and PV, respectively.
    if i==min(manifold_range):
        ZCT_matrix = np.column_stack((new_Z, T_C_group)) # Adds the 100x2 T/C array to the 100x1 Z array on the 1st flamelet.
        ...
        ZCT_matrix   = np.column_stack((ZCT_matrix, T_C_group)) # Adds the newest 100x2 solution array to the library.
...
# Normalize the PV (1st column) within each constant-Z "slice": PV_norm = (PV - PV_min) / (PV_max - PV_min). Then, interpolate φ on a rectilinear grid of the PV.
    PV_T_norm = (Soln_2D_T[:,1] - min(Soln_2D_T[:,1]) )/(max(Soln_2D_T[:,1]) - min(Soln_2D_T[:,1])) # Get normalized PV using the PV range within constant-Z as a reference. PV_norm = (PV - PV_min) / (PV_max - PV_min)    
